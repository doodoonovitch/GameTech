...
void Engine::InternalInitialize(GLint viewportX, GLint viewportY, GLsizei viewportWidth, GLsizei viewportHeight, GLsizei gBufferWidth, GLsizei gBufferHeight)
{
	if (!mInitialized)
	{
		...
		mPointLightHelperRenderer.Initialize();
		glUniform1f(mPointLightHelperRenderer.mShader.GetUniform(Renderers::BasicTessSphereShader::u_TessFactor), 3.f);
		PRINT_MESSAGE("... end.\n");
...
void Engine::InternalCreateGBuffers()
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, mGBuffers[gBuffer_NormalBuffer], 0);
	GL_CHECK_ERRORS;
...
void Engine::InternalInitializeDeferredPassShader()
{
	...
	glUniform1i(mDeferredShader.GetUniform(u_gBufferNormal), 4);
