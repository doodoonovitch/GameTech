...
{	
    ...
    for(int i = 0; i < u_KernelSize; ++i)
    {
		...
		viewRay = offset.xy / vec2(u_ProjMatrix[0][0], u_ProjMatrix[1][1]);

        offset.xyz = offset.xyz * 0.5 + 0.5; 
        float sampleDepth = texture(u_gDepthMap, offset.xy).r; 
		vec3 position2 = PositionFromDepth(sampleDepth, viewRay);
        
        // range check & accumulate
        float rangeCheck = smoothstep(0.0, 1.0, u_Radius / abs(position.z - position2.z));
